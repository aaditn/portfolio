    ...
    <div className="min-h-screen bg-white text-black selection:bg-blue-100">
      <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-gray-100">
        <div className="max-w-4xl mx-auto px-6 py-4 flex justify-between items-center">
          <Link href="/" className="text-black hover:text-blue-600 font-bold transition-colors flex items-center gap-2">
      ...
      <main className="max-w-3xl mx-auto px-6 py-12 md:py-24">
        <header className="mb-16">
            ...
            {/* Added truncate for one-line and mb-12 for spacing */}
            <h1 className="text-3xl md:text-3xl font-black text-black mb-16 leading-[1.1] tracking-tight truncate">
              {projectData.title}
            </h1>
            
            <div className="flex flex-wrap gap-2 mb-12">
                {projectData.labels.map((label: string) => (
                    <span key={label} className="bg-gray-100 border border-gray-200 text-black px-3 py-1.5 rounded-lg text-xs font-bold">
                        {label}
                    </span>
                ))}
            </div>
            ...
            <div className="bg-gray-50 p-8 rounded-3xl border border-gray-100 shadow-sm">
              <p className="text-xl md:text-2xl text-gray-800 leading-relaxed font-medium italic">
                  {projectData.summary} {/* Quotes removed here */}
              </p>
            </div>
        </header>
        ...
        <div className="mt-24 pt-12 border-t border-gray-200 text-center">
            <Link href="/" className="inline-block bg-black text-white px-10 py-4 rounded-full font-black hover:bg-blue-600 transition-all shadow-xl hover:-translate-y-1">
                Explore More Work
            </Link>
        </div>
      </main>
